...
class CreateBizSusuScreen extends StatefulWidget {
  const CreateBizSusuScreen({super.key});

  @override
  State<CreateBizSusuScreen> createState() => _CreateBizSusuScreenState();
}

class _CreateBizSusuScreenState extends State<CreateBizSusuScreen> {
  ...
  void createSusu() async {
    ...
    try {
      final response = await ApiService().createBizSusu(
  ...
  void approveSusuCreation() async {
    showLoadingDialog(context);
    try {
      final response = await ApiService().approveBizSusu(linkedAccountResource,
